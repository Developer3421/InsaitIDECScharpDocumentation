...
<p align="center">
  <a href="#english">🇬🇧 English</a> · <a href="#ukrainian">🇺🇦 Українська</a> · <a href="#deutsch">🇩🇪 Deutsch</a>
</p>
...
This project is licensed under the **MIT License (with Exclusions)** — see the [LICENSE](LICENSE) file for details.

- ✅ You **may** use, copy, modify, and distribute the source code, provided you include a clear attribution to the original project ([Insait Edit — C# IDE](https://github.com/Developer3421/InsaitIDECScharpDocumentation)).
- ❌ The **application UI styles**, **themes**, **icons**, and **custom graphic elements** are **excluded** from the MIT license and remain **All Rights Reserved**. They may not be used, reproduced, or distributed without explicit written permission.

See [LICENSE](LICENSE) for the full terms and the complete exclusion list.
...
**Insait Edit** — це Windows-десктопна IDE, орієнтована на розробку на C# і .NET. Застосунок поєднує кастомний редактор, аналіз на основі Roslyn, інструменти створення проектів, робочі процеси збірки та публікації, інтеграцію з Git, вбудований термінал та інструменти пакування MSIX в єдиному інтерфейсі.

Кодова база зосереджена на робочих процесах настільних застосунків. Включено вбудоване створення проектів для **Avalonia**, **Windows Forms**, стандартні шаблони **консоль/бібліотека класів** та **стартові шаблони F#**. Конвеєр збірки та запуску також розпізнає **WPF**, **WinForms** і **Avalonia** як GUI-проекти.

---

## Основні можливості

### Повний цикл роботи з десктопом
- Створення нових рішень та проектів з IDE
- Додавання проектів та елементів до наявного рішення
...
### Підтримувані проектні робочі процеси
- **Avalonia-застосунки** з підтримкою редагування AXAML та попереднього перегляду
- **Windows Forms застосунки** через шаблони проектів і виявлення GUI-проектів
- **WPF-проекти** у конвеєрі збірки/запуску
- **Консольні застосунки**, **бібліотеки класів**, **порожні C#-проекти** та **стартові F#-проекти**

### Редактор та інтелектуальний аналіз коду
- Кастомний `Insait Editor` із підсвічуванням синтаксису та власним рендерингом поверхні редактора
- Roslyn-автодоповнення C#, швидка інформація, пошук символів і помічники рефакторингу
- Підтримка доповнення F# та підказок через `FSharp.Compiler.Service`
- Доповнення AXAML/XAML для елементів Avalonia, властивостей, подій, розширень розмітки та просторів імен
- Inline-діагностика, пропозиції Quick Fix та Auto Fix прямо в редакторі

### Збірка, запуск та публікація
- Інтеграція MSBuild / `dotnet` для операцій збірки
- Конфігурації запуску та підтримка комбінованого запуску
- Конвеєр запуску з підтримкою GUI для десктопних проектів
- Вікно публікації з візуальним відображенням прогресу

### Управління пакетами та розгортання
- Панель NuGet для перегляду, встановлення, оновлення та видалення пакетів
- MSIX-менеджер для пакування, редагування метаданих, заміни іконок та підпису

### Управління вихідним кодом та інструменти GitHub
- Панель Git для статусу репозиторію, зафіксованих/незафіксованих змін, інформації про гілки, історії, коміту, push та pull
- Робочі процеси клонування репозиторіїв
- Інтеграція з GitHub через сервіси на базі Octokit
- Вбудована панель **GitHub Copilot CLI** та робочий процес команд

### Термінал та утиліти робочого простору
- Вбудована термінальна панель із керуванням процесами та історією команд
- Провідник файлів, пошук у файлах, переглядач зображень, сповіщення, інструменти кодування та вікна властивостей проекту
- Вікна попереднього перегляду AXAML та інструменти live-host попереднього перегляду

### Локалізація
- Вбудовані UI-словники для **англійської, української, німецької, російської та турецької** мов
- Кастомні словники AXAML, що завантажуються під час виконання
- Імпорт зовнішніх `.axaml`-файлів перекладу через меню мови
- Папка перекладів для ручного редагування або перекладу за допомогою GitHub Copilot CLI

---

## Система аналізу та доповнення Roslyn

Основний досвід кодування базується на **Microsoft Roslyn** та кількох спеціалізованих сервісах у проекті.

### Що реалізовано
- `RoslynAutoCompleteFactory` делегує C#-доповнення безпосередньо до Roslyn `CompletionService`
- Допомога із сигнатурами вирішується з семантичних даних Roslyn
- Hover-інформація використовує Roslyn `QuickInfoService`
- `InlineDiagnosticService` виконує фоновий аналіз з дебаунсом та оновлює inline-підкреслення
- Пропозиції Quick Fix прикріплюються до діагностик і відображаються в редакторі
- `RoslynAutoFixService` виявляє вбудовані реалізації Roslyn `CodeFixProvider` та `CodeRefactoringProvider`
- `CSharpCompletionService` підтримує перейменування символів та підсвічування в документі

### Практичний результат в IDE
- розумні списки доповнення
- допомога з параметрами та перевантаженнями
- hover-описи
- звіти про помилки/попередження в реальному часі
- Quick Fix в один крок
- перехід до визначення
- перейменування символу

Для файлів AXAML IDE використовує окремий движок доповнення, який відображає реальні Avalonia-збірки та пропонує валідні елементи керування, властивості, прикріплені властивості, події та розширення розмітки.

---

## Нотатки щодо пакування MSIX

Підсистема MSIX підтримує як **повне пакування**, так і **пакування з вже опублікованого виводу**.

### Що вміють MSIX-інструменти
- опублікувати проект та упакувати його в MSIX в одному потоці
- генерувати `AppxManifest.xml`
- пакувати вміст за допомогою `MakeAppx.exe`
- відкривати наявний MSIX та читати метадані пакету
- редагувати метадані пакету та перепаковувати його
- замінювати іконки всередині наявного MSIX
- підписувати MSIX за допомогою `SignTool.exe` та сертифіката з `CurrentUser\My`

### Важливі вимоги
- для пакування MSIX **потрібен Windows SDK**, оскільки використовуються `MakeAppx.exe` та `SignTool.exe`
- **Publisher пакету повинен точно збігатися із суб'єктом сертифіката**
- на практиці це означає, що **ім'я видавця `CN=...` повинно бути ідентичним distinguished name сертифіката**
- іконки пакету для MSIX мають бути **у форматі PNG**
- якщо дійсний логотип не вказано, сервіс може повернутися до зображення-заглушки

### Рекомендації щодо використання
- явно встановлюйте власну іконку пакету; значення за замовчуванням — лише заглушка
- перевіряйте ідентифікатор пакету, видавця, версію, виконуваний файл та точку входу перед підписанням
- якщо підписання не вдається через невідповідність видавця, узгодьте видавця маніфесту з суб'єктом сертифіката

---

## Локалізація та кастомні переклади

Кастомні переклади керуються через прості AXAML-словники.

### Вбудована поведінка
- Стандартні мови інтерфейсу завантажуються з `Insait Edit C Sharp/Interface Localization/`
- Кастомні словники зберігаються в `%AppData%\InsaitEdit\GitHubTranslations\`
- Сервіс забезпечує доступність копії `English.axaml` як базового шаблону

### Правила для кастомних перекладів
- файл кастомної мови повинен бути простим `.axaml`-словником
- він повинен зберігати **ту саму структуру ключів `x:String`**, що й англійський словник
- значення можна перекладати, але ключі та структура повинні залишатися ідентичними `English.axaml`

### Як кастомні переклади використовуються в застосунку
- імпортуйте зовнішній AXAML-файл через меню мови
- відкрийте папку перекладів і редагуйте файли вручну
- запустіть **GitHub Copilot CLI** у цій папці для допомоги з перекладом
- завантажте кастомний словник безпосередньо з меню мови під час виконання
...
| `Ctrl+P` | Пошук файлу за ім'ям |
| `Ctrl+Shift+Z` | Перемкнути Zen Mode |
| `Ctrl+Shift+P` | Відкрити попередній перегляд AXAML |
| `Ctrl+Shift+E` | Перемкнути панель провідника |
| `Ctrl+Shift+I` | Перемкнути AI / праву панель |
| `Ctrl+\`` | Перемкнути нижню панель / термінал |
| `Esc` | Вийти з Zen Mode |

### Редактор
| Скорочення | Дія |
|---|---|
| `Alt+Enter` | Показати Quick Fix на позиції курсора |
| `Ctrl+.` | Показати Quick Fix на позиції курсора |
| `Ctrl+Shift+I` | Форматувати документ |
| `Ctrl+Shift+A` | Відкрити вікно Auto Fix |
| `F12` | Перейти до визначення |
| `F2` | Перейменувати символ |
| `Ctrl+R` | Перейменувати символ |
| `Ctrl+Shift+H` | Показати hover-інформацію |
| `Tab` / `Enter` | Підтвердити вибраний елемент доповнення |
| `Esc` | Закрити доповнення або Quick Fix |

### Термінальна панель
| Скорочення | Дія |
|---|---|
| `Ctrl+C` | Зупинити поточний процес термінала |
| `Ctrl+L` | Очистити вивід термінала |
| `↑ / ↓` | Переміщатися по історії термінала |
...
| Build-шар | `Microsoft.Build` 18.4 |
| Інтеграція NuGet | `NuGet.Protocol` 7.3 |
| Інтеграція GitHub | `Octokit` 14.0 |
| Локальне сховище | `LiteDB` 6 prerelease |

---

## Ліцензія

Цей проект ліцензовано на умовах **Ліцензії MIT (з винятками)** — детальніше в файлі [LICENSE](LICENSE).

- ✅ Ви **можете** використовувати, копіювати, змінювати та розповсюджувати вихідний код за умови чіткого зазначення джерела — посилання на оригінальний проект ([Insait Edit — C# IDE](https://github.com/Developer3421/InsaitIDECScharpDocumentation)).
- ❌ **Стилі UI застосунку**, **теми**, **іконки** та **кастомні графічні елементи** є **виключеними** з ліцензії MIT і залишаються **All Rights Reserved**. Їх використання, відтворення або розповсюдження без явного письмового дозволу заборонено.

Повні умови та перелік виключених активів — у файлі [LICENSE](LICENSE).
...
Dieses Projekt steht unter der **MIT-Lizenz (mit Ausnahmen)** — Details finden Sie in der Datei [LICENSE](LICENSE).

- ✅ Sie **dürfen** den Quellcode verwenden, kopieren, ändern und verteilen, sofern eine klare Quellenangabe mit Verweis auf das Originalprojekt ([Insait Edit — C# IDE](https://github.com/Developer3421/InsaitIDECScharpDocumentation)) enthalten ist.
- ❌ Die **UI-Stile**, **Themes**, **Icons** und **benutzerdefinierten grafischen Elemente** der Anwendung sind von der MIT-Lizenz **ausgenommen** und verbleiben als **All Rights Reserved**. Ihre Verwendung, Vervielfältigung oder Verbreitung ohne ausdrückliche schriftliche Genehmigung ist untersagt.

Die vollständigen Bedingungen und die Ausschlussliste finden Sie in der Datei [LICENSE](LICENSE).
